...
const playerNickSpan = document.getElementById("player-nick-span");
...
startButton.addEventListener("click", () => {
    playerNick = playerNickInput.value;
    playerNickSpan.textContent = playerNick;

    const file = playerPhotoInput.files[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = (e) => {
            playerPhotoImg.src = e.target.result;
            playerPhotoImg.style.display = "block";
            startGame();
        };
        reader.readAsDataURL(file);
    } else {
        playerPhotoImg.style.display = "none";
        startGame();
    }
});
