(function () {
  ...
  var CollisionDetector = window.PongGameFour.CollisionDetector = function (player, ball, context) {
    this.player = player;
    this.ball = ball;
  ...
  CollisionDetector.prototype.checkCollision = function() {
    if ((this.player.side == "left" || this.player.side == "left1" ) && this.hitLeft()) {
      this.ball.changeBallDirection()
      this.ball.hits += 1;
      this.ball.newHit += 1;
    } else if ((this.player.side == "right" ||  this.player.side == "right1")  && this.hitRight()) {
      this.ball.changeBallDirection()
      this.ball.hits += 1;
      this.ball.newHit += 1;
    }
  ...
  CollisionDetector.prototype.score = function () {
    if ((this.player.side == "right" || this.player.side == "rigth1") && this.ball.isLeft()) {
    ...
    } else if ((this.player.side == "left" || this.player.side == "left1") && this.ball.isRight()) {
              this.player.points +=1;
              this.ball.resetIncrement();
              //this.ball.changeBallDirection();
              this.ball.position[0] = this.context.canvas.width / 2;
